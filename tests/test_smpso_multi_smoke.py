...
class SMPSOMultiSmokeTest(unittest.TestCase):
    def test_multi_smpso_writes_artifacts(self) -> None:
        project_root = Path(__file__).resolve().parent.parent
        terrain = load_terrain_struct(project_root / "problems" / "terrainStruct_c_100.mat")
        terrain["n"] = 3
        multi = make_multi_uav_terrain(terrain, fleet_size=3, seed=17, separation_min=10.0)
