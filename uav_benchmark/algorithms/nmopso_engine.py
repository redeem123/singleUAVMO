"""Self-contained NMOPSO engine for multi-UAV path planning.

This module is **Layer 1** of the 3-layer architecture:
  Layer 1 — PSO mechanics (this file)
  Layer 2 — RL adaptive controller (rl_pso_adapter.py)
  Layer 3 — Experiment runner       (multi_uav.py)

The engine owns all PSO state (population, velocity, pbest, archive) and
exposes a simple interface for the RL controller:

    engine = NMOPSOEngine(...)
    result = engine.step(w, c1, c2, ...)            # core PSO update
    n = engine.inject_sbx(ratio, replace_ratio, ...)  # operator arm 1
    n = engine.inject_de(f, cr, ratio, ...)           # operator arm 2
    n = engine.elite_refine(sigma, top_k, iters)      # operator arm 3
    engine.update_archive()                           # Pareto/R2 prune
    feat = engine.state_features()                    # 6-dim for RL
"""
...
# ── Helpers (moved from multi_uav.py) ──────────────────────────────
...
def _normalize_rl_feature_mode(raw: Any) -> str:
...
class NMOPSOEngine:
    """Stateful NMOPSO engine for multi-UAV path planning.

    Owns: population, velocity, pbest, archive, evaluation.
    Exposes ``step()`` + operator arms for the RL adapter to call.
    """
    ...
    def __init__(
        ...
        self.rl_feature_mode = _normalize_rl_feature_mode(rl_feature_mode)
    ...
    def step(
        ...
        if self.archive:
            ...
            if leader_vectors is None:
                # Fallback to standard grid/random selection if no RL attention
        ...
        if mutation_prob > 1e-9:
            ...
            if self.paper_nmopso:
                ...
                if np.any(mutation_mask):
                    ...
                    sigma_scale = (
                        RLD.MUTATION_SIGMA_HIGH * (1.0 - gen_progress) + RLD.MUTATION_SIGMA_LOW
                        if self.is_nmopso_family
                        else RLD.MUTATION_SIGMA_GENERIC
                    )
        ...
        # --- Targeted Collision Repair (Multi-UAV Heuristic) ---
        if self.fleet_size > 1 and hasattr(self, "candidates") and self.candidates:
            # Only apply proportional repair to highly constrained multi-uav spaces
    ...
    def state_features(
        ...
        """Build the 6-dim RL state vector."""
        finite_archive = self._finite_archive_matrix()
        if self.hv_ref_point is None and finite_archive.size > 0:
            self.hv_ref_point = _fixed_hv_reference(finite_archive)
