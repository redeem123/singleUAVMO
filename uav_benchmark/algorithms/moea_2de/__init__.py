...
def _fair_shim_root() -> Path:
    return _bridge_root() / "moea2de_fair_shims"


def _reference_root() -> Path:
    return _python_root() / "research" / "reference_code" / "uav_comparators" / "MOEA-2DE" / "extracted" / "MOEA-2DE"
...
def _model_for_reference(model: dict[str, Any], start: np.ndarray, goal: np.ndarray) -> dict[str, Any]:
    out: dict[str, Any] = {}
    for key in ("H", "xmin", "xmax", "ymin", "ymax", "zmin", "zmax", "safeH"):
        out[key] = model[key]
...
def _run_reference_matlab(
    *,
    ...
    if not reference_root.exists():
        raise RuntimeError(f"MOEA-2DE reference code not found at {reference_root}")

    work_dir = run_dir / "_moea2de_reference"
    ensure_dir(work_dir)
    output_path = work_dir / "moea2de_result.mat"
    config_path = work_dir / "moea2de_config.mat"
    context_path = work_dir / "moea2de_context.pkl"
    ...
    config = {
        ...
        "pathNodeCount": int(max(3, n_waypoints + 2)),
        "problemIndexForReference": int(_reference_problem_index(params.problem_name)),
        "F": float(params.extra.get("moea2deF", 0.3)),
        "pc": float(params.extra.get("moea2deCrossoverProbability", 0.9)),
        "pm": float(params.extra.get("moea2deMutationProbability", 0.1)),
    ...
    statement = f"addpath('{bridge_root}'); run_moea2de_reference_bridge('{config_arg}')"
    ...
    if completed.returncode != 0:
        raise RuntimeError(f"MOEA-2DE MATLAB run failed. See {work_dir / 'matlab_stderr.txt'}")
    result = load_mat(output_path)
    return _paths_from_stack(np.asarray(result.get("PathStack", np.zeros((0, 0, 3))), dtype=float))


def run_moea_2de(model: dict[str, Any], params: BenchmarkParams) -> np.ndarray:
    model = dict(model)
    n_waypoints = int(model.get("n", 10))
    requested_fleet = max(1, int(params.fleet_size or model.get("fleetSize", 1)))
    if requested_fleet != 1:
        raise RuntimeError("MOEA-2DE official reference adapter supports only single-UAV problems.")
    ...
    for run_idx in _resolve_run_indices(params):
        ...
        _save_fleet_artifacts(
            ...
            gpu_backend="matlab:moea-2de-reference",
            gpu_peak_bytes=0.0,
            run_metadata={
                "algorithmName": "MOEA-2DE",
                "optimizerBackend": "Official MATLAB MOEA-2DE operators with batched benchmark objective shim",
