...
function [ok, pathSet, pathNames] = pick_paths_for_plot(problemDir, runDirs)
    ...
    % Fallback: choose one feasible path from Run_1 bp files.
    ...
    feasibleIdx = feasible_indices_for_bp(runOne, countHint);
...
function idx = feasible_indices_for_bp(runDir, countHint)
    idx = [];
    if countHint <= 0
        return;
    end
...
def generate_research_plots(project_root: Path, results_dir: Path | None = None) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="uav_matlab_plot_driver_") as tmp_dir:
        ...
        driver_file = driver_dir / "generate_research_plots_cli.m"
        driver_file.write_text(_MATLAB_DRIVER_SOURCE, encoding="utf-8")
        matlab_cmd = (
            f"addpath('{_quote_matlab_string(str(driver_dir))}'); "
