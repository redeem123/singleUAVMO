...
from uav_benchmark.analysis.metrics.compute import _build_ref_points
from uav_benchmark.analysis.metrics.feasibility import (
    finite_rows_mask,
    load_mission_feasible_mask,
    normalize_pop_obj,
)
...
def _load_popobj_raw(run_dir: Path) -> np.ndarray:
    ...
    pop_obj = normalize_pop_obj(pop_obj, objective_count=4)
    if pop_obj.size > 0:
        return pop_obj
...
def _load_feasible_mask(run_dir: Path, pop_obj: np.ndarray) -> np.ndarray:
    ...
    finite_mask = finite_rows_mask(pop_obj) if count > 0 else np.zeros(0, dtype=bool)
    if count == 0:
        return finite_mask
    return load_mission_feasible_mask(run_dir, count=count, base_mask=finite_mask)
