...
from uav_benchmark.analysis.metrics.feasibility import load_mission_feasible_mask, normalize_pop_obj
...
def _sanitize_popobj(pop_obj: np.ndarray, objective_count: int | None) -> np.ndarray:
    if pop_obj.size == 0:
        return pop_obj
    pop_obj = normalize_pop_obj(pop_obj, objective_count=objective_count)
    pop_obj = pop_obj[np.all(np.isfinite(pop_obj), axis=1)]
    return pop_obj


def _load_feasible_mask(run_dir: Path, count: int) -> np.ndarray:
    return load_mission_feasible_mask(run_dir, count=count)
