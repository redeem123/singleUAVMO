...
%--------------------------------------------------------------------------
    ...
    [N,M]  = size(PopObj);
    MaxPopObj = MaxPopObj(:)'; % Force to row vector
    fmin   = min(min(PopObj,[],1),zeros(1,M));
    fmax   = max(MaxPopObj,zeros(1,M));
    
    % Use implicit expansion for normalization
    % (PopObj - fmin) ./ ((fmax - fmin) * 1.1)
    denom = (fmax - fmin) * 1.1;
    denom(denom == 0) = 1; % Avoid division by zero
    PopObj = (PopObj - fmin) ./ denom;
