...
Unmanned aerial vehicles (UAVs) are increasingly used in tasks such as inspection, delivery, mapping, surveillance, and reconnaissance. Civil-application and path-planning surveys show that these missions require more than shortest geometric routes \cite{otto2018uav_civil,aggarwal2020uav_techniques}. Environment-complexity and evolutionary-computation surveys emphasize terrain, clearance, altitude, smoothness, and operational constraints \cite{jones2023uav_survey,jiang2024ec_uav_survey}. Recent UAV reviews and benchmark studies tie the same concerns to reproducible multi-objective evaluation \cite{meng2025uav_advances,darlan2025benchmark}. The difficulty increases further when several UAVs fly in a shared environment. Collaborative trajectory-planning and swarm-planning reviews emphasize coordination and separation constraints for multi-UAV missions \cite{wang2024collaborative_review,li2026swarm_review}. Multi-UAV optimization studies then turn those interactions into explicit constraint-handling and multitasking objectives \cite{zhang2024twofold_uav,meng2025emmop}.

Multi-objective formulations are a natural fit for this setting because improving one criterion can degrade another. A path that shortens distance may pass closer to terrain or other UAVs. A path that increases clearance may require sharper turns or more energy. Early comparative work already argued that UAV planners should be judged by problem-specific multi-objective indicators rather than one scalar route cost \cite{besada2013uav_comparison}. Decomposition-based and dimension-exploration UAV studies make the same trade-off structure explicit in constrained evolutionary settings \cite{peng2022decomp_uav,xu2024dimension_uav}. Urban many-objective planning and navigation-variable PSO studies further show that path quality, safety, and kinematics must be evaluated together \cite{hohmann2023urban_uav,duong2025nmopso_uav}. Recent constrained-UAV reviews and benchmark papers reinforce the need for safety diagnostics alongside objective scores \cite{huang2024uav_cmoea_review,darlan2025benchmark}. Multi-UAV and domain-specific operator studies add the fleet and operator-design pressures that motivate this work \cite{meng2025emmop,zhou2024tskac}. The local domain-operator reference also motivates comparing CGPO against UAV-specific evolutionary mechanics rather than only generic MOEAs \cite{darlan2026domainops}.
...
The remainder of the paper is organized into six sections. Section \ref{sec:related-work-problem} reviews related work and formulates the benchmark problem. Section \ref{sec:method} describes CGPO. Section \ref{sec:experimental-protocol} gives the experimental protocol. Section \ref{sec:results} reports the reduced matched comparison, focused validation evidence, ablation results, and trace analysis. Section \ref{sec:conclusion} summarizes the current evidence, reproducibility package, limitations, and data availability.

\section{Related work and problem formulation}
\label{sec:related-work-problem}

\subsection{Related work}

UAV path planning has been studied with graph search, sampling methods, potential fields, swarm intelligence, and evolutionary computation. Broad UAV surveys organize these techniques around mission requirements, environment complexity, and operational safety \cite{otto2018uav_civil,aggarwal2020uav_techniques}. More recent surveys focus on environment complexity and evolutionary computation as recurring sources of route-planning difficulty \cite{jones2023uav_survey,jiang2024ec_uav_survey}. A recent UAV-planning review extends the same discussion to current algorithmic challenges and future directions \cite{meng2025uav_advances}. For complex environments, multi-objective approaches are useful because decision makers often need a set of trade-off paths rather than one weighted-sum solution. Recent search-based multi-objective path-planning frameworks formalize this trade-off view beyond UAV-only settings \cite{ren2025emoa}. Early comparative work emphasized the need for problem-specific quality indicators and statistical characterization when comparing multi-objective UAV planners \cite{besada2013uav_comparison}. More recent benchmark work frames UAV path planning around conflicting safety, efficiency, and regulatory constraints and introduces standardized urban, suburban, and mountainous scenarios \cite{darlan2025benchmark}. A constrained-UAV mission-planning review reaches a similar conclusion from the perspective of constrained MOEA design \cite{huang2024uav_cmoea_review}.

Recent multi-UAV work strengthens the same direction. Two-fold constraint handling has been studied for multiple-UAV path planning \cite{zhang2024twofold_uav}. Knowledge-assisted coevolution has been used for bi-objective multi-UAV planning \cite{zhou2024tskac}. Evolutionary multitasking has also been used for multiobjective multi-UAV path planning with adaptive operator selection and knowledge fusion \cite{meng2025emmop}. Other local-corpus studies emphasize dimension-discrepancy search and urban many-objective planning for UAV routes \cite{xu2024dimension_uav,hohmann2023urban_uav}. Navigation-variable PSO and domain-specific evolutionary operators provide two further UAV-specialized design points \cite{duong2025nmopso_uav,darlan2026domainops}. These works motivate the present paper's fleet setting: single-UAV path quality is insufficient when inter-UAV constraints become part of the mission.
...
Constrained multi-objective optimization requires algorithms to handle objective trade-offs and constraint satisfaction simultaneously. General MOEA and constraint-handling surveys establish the core tension between convergence, diversity, and feasibility \cite{zhou2011moea_survey,mezura2011constraint_survey}. Push-pull search and constrained-MOEA test-suite work show how feasibility pressure can be staged or benchmarked systematically \cite{fan2019pushpull,ma2019cmop_test}. A more recent constrained-MOEA survey summarizes how archives, ranking rules, and auxiliary designs affect search behavior \cite{liang2023cmop_survey}. Multitasking-based constrained optimization studies show that knowledge transfer and constraint subsets can also influence constrained search \cite{ming2024emt_cmop,yu2024constraint_emt}. The CGPO design is closest to constraint-handling methods that treat infeasible and boundary candidates as informative rather than simply invalid.
...
HV and IGD+ measure Pareto-set quality, while the mission diagnostics prevent unsafe paths from being hidden behind aggregate objective scores. The use of multiple indicators follows established multiobjective performance-assessment guidance \cite{zitzler2003performance}. Hypervolume is included because it is a standard Pareto-set quality indicator with dedicated algorithmic treatment \cite{while2006hypervolume}. UAV planner comparison work also supports reporting problem-specific diagnostics rather than relying on one aggregate indicator \cite{besada2013uav_comparison}. This follows the evidence style of SWEC UAV papers, where tables, convergence, path visualizations, and safety diagnostics are used together rather than relying on one metric.
...
\begin{figure*}[t]
\centering
\resizebox{\textwidth}{!}{\input{figures/cgpo_method_tikz.tex}}
...
\subsection{Graph-guided evolutionary mechanisms}
...
The reduced matched comparison uses CGPO, NSGA-II, MOEA/D, and NMOPSO as MOEA-style optimization rows. The run budget is five independent runs, 500 generations, requested population 40, seed 11, fleet mode, fleet size 3, the \texttt{c\_100}, \texttt{m\_100}, and \texttt{s\_120} problems, the \texttt{paper\_medium} scenario set, GPU mode off, metric interval 10, and \texttt{nRep: 40}. MOEA/D's NBI weight generation uses 35 internal weight vectors for the requested 40-population budget; this is recorded as a protocol note rather than hidden. Larger-scope methods such as MOEA-2DE, EMMOP, CCEA-ADVS, TSKAC-NSGA-II, and DTAPP-IICR remain part of the planned full protocol, but they are not used for reduced-budget claims in this manuscript. The protocol is included because multiobjective comparison requires transparent indicator reporting \cite{zitzler2003performance}. It also follows recent UAV benchmark practice by fixing scenarios, budgets, baselines, and problem definitions \cite{darlan2025benchmark}. The multi-UAV and domain-operator references motivate keeping fleet-aware and UAV-specialized baselines in the broader comparison set \cite{meng2025emmop,darlan2026domainops}.
...
\subsection{Ablation and trace analysis}
...
\label{sec:reproducibility-limitations}
